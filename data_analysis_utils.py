...
# Identifies lineages with at least two distinct IF sequences, and prepares sequences for tree analysis
def prepare_sequences_for_trees_old(df_IGH, df_IGH_IF, df_combined, use_EF=False, write_files=False):
    min_lineage_size = 2
    lineage_lens = []
    lineage_ids = []
    N_lineages_tot = 0
    ...
    if write_files:
        if not use_EF:
            ...
            for lineage_id, group in df_IGH_IF.groupby('lineage_id'):
                ...
                group_trimmed = group.drop_duplicates(subset='v_mutations')
                if len(group_trimmed) >= min_lineage_size:
                    filename = os.path.join(output_dir, f"lin_{int(float(lineage_id))}_no_ef.fasta")
                    with open(filename, "w") as fasta_file:
                        seq_len = None
                        for idx, row in group_trimmed.iterrows():
                            if not isinstance(row['v_mutations'], str):
            ...
            output_dir = "Lineage_Seqs_Final_EF"
            os.makedirs(output_dir, exist_ok=True)
...
# Import data
def data_readin_old(tonsil_vdjs_path, metadata_path, UMI_collapsed_path, clone_list_path):

    with open(tonsil_vdjs_path, 'r') as file:
        df = pd.read_csv(file, sep='\t', dtype=str)

    # Matching section IDs for EF/IF labels
    with open(metadata_path, 'r') as file:
        metadata_df = pd.read_csv(file, sep=',', dtype=str)
    metadata_df['plasma_frac'] = metadata_df['Plasmablast'].astype(float) / (metadata_df['Plasmablast'].astype(float) + metadata_df['B.Cells'].astype(float))

    # Focus on heavy chain reads, splitting into intra- and extrafollicular
    df_IGH = df[df['locus'] == 'IGH'].copy()

    # Also read in T cell receptors
    df_collapsed = pd.read_csv(UMI_collapsed_path)
    df_clones = pd.read_csv(clone_list_path)
    cloneid_to_type = df_clones.set_index('cloneId')['type']
    df_collapsed['type'] = df_collapsed['cloneId'].map(cloneid_to_type)
    ...
    max_id = 0
    for lineage_id, group in df_IGH.groupby('lineage_id'):
        max_id = max(max_id, int(float(lineage_id)))
    for lineage_id, group in df_IGH.groupby('lineage_id'):
        v_calls = group['v_db_call'].unique()
        if len(v_calls) > 1:
            minority_v_call = v_calls[1]
            df_IGH.loc[(df_IGH['lineage_id'] == lineage_id) & (df_IGH['v_db_call'] == minority_v_call), 'lineage_id'] = str(float(max_id + 1))
            max_id += 1

    df_IGH_IF = df_IGH[df_IGH['follicle'] != 'EF'].copy()
    df_IGH_EF = df_IGH[df_IGH['follicle'] == 'EF'].copy()
    df_IGH_EF_PBdom = df_IGH_EF[df_IGH_EF['plasma_frac'] > 0.9].copy()

    # Realign sequences with inconsistent lengths
    df_IGH_IF.loc[201, 'v_sequence_no_trunc'] = 'NN' + df_IGH_IF.loc[201]['v_sequence_no_trunc']
    df_IGH_IF.loc[36307, 'v_sequence_no_trunc'] = 'NN' + df_IGH_IF.loc[36307]['v_sequence_no_trunc']
    df_IGH_IF.loc[25566, 'v_sequence_no_trunc'] = 'NNNNNN' + df_IGH_IF.loc[25566]['v_sequence_no_trunc']
    df_IGH_EF_PBdom.loc[18826, 'v_sequence_no_trunc'] = 'N' + df_IGH_EF_PBdom.loc[18826]['v_sequence_no_trunc']
    ...
    for lineage_id, group in df_combined.groupby('lineage_id'):
        ...
        if len(unique_root_seqs) > 1:
            ...
            for seq, count in unique_root_seqs.items():
                tot_ct += count
            for seq, count in unique_root_seqs.items():
                if count/tot_ct > max_ct:
                    max_ct = count/tot_ct
                    consensus_seq = seq
            for idx, row in group.iterrows():
                if get_root_seq(row) != consensus_seq:
                    df_combined.at[idx, 'v_mutations'] = get_muts_from_consensus_seq(row['v_sequence_no_trunc'], consensus_seq)

    df_IGH_IF = df_combined[df_combined['follicle'] != 'EF'].copy()
    df_IGH_EF_PBdom = df_combined[df_combined['follicle'] == 'EF'].copy()
